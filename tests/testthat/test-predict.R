...
test_that("Outputs the correct values with default parameters", {
  function_output <- glm_predict(cancer_model)
...
test_that("Works with optional parameters on existing data in model", {
  train_0.8_output <- glm_predict(cancer_model, threshold=0.8)
...
test_that("Works with optional parameters on new data", {
  test_0.4_output <- glm_predict(cancer_model, threshold=0.4, newdata=cancer_test)
...
test_that("Throws errors if improper parameter values are given", {
  expect_error(glm_predict(1:20))  # Improper model type
  expect_error(glm_predict(cancer_model, threshold="half"))  # Improper data type for threshold
